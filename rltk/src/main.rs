use rltk::{GameState, Rltk, RGB, RltkBuilder, VirtualKeyCode};
use specs::prelude::*;
use std::cmp::{max, min};
...
fn try_move_player(delta_x: i32, delta_y: i32, ecs: &mut World) {
    ...
    for (_player, pos) in (&mut players, &mut positions).join() {
        ...
        if map[destination_idx] != TileType::Wall {
            pos.x = min(79 , max(0, pos.x + delta_x));  // TODO: Magic numbers. Make const
...
pub fn xy_idx(x: i32, y: i32) -> usize {
    (y as usize * 80) + x as usize  // TODO: Magic number. Make const.
}
